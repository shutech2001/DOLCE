...
def train_reward_model(
    ...
    g_hat = train_pair_model(dataset=dataset, random_state=random_state)
    reward = dataset["r"].astype(float)
    reward_lag = reward - g_hat[np.arange(dataset["x_t"].shape[0]), dataset["a_t"]]
    h_hat = fit_predict_by_MLP_for_all_actions(
        dataset["x_t_l"],
        rewards=reward_lag,
        num_actions=dataset["num_actions"],
    )
...
def fit_predict_by_MLP(
    ...
    """
    Estimate the reward function using an MLP
    """
    model = MLPRegressor(
...
def fit_predict_by_MLP_for_all_actions(
    ...
    for action in range(num_actions):
        action_column = np.full((features.shape[0], 1), action)
        X = np.hstack((features, action_column))
        scaler = StandardScaler()
        X_ = scaler.fit_transform(X)
        model.fit(X_, rewards)
        q_hat_one_column = model.predict(X_)
        if action == 0:
            pred_result = q_hat_one_column[:, None]
        else:
            pred_result = np.hstack((pred_result, q_hat_one_column[:, None]))
    return pred_result
